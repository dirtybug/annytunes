...
public class AnytoneUart implements AutoCloseable {
    ...
    public static synchronized AnytoneUart getObj() {
        if (instance == null) throw new IllegalStateException("AnytoneUart not initialized; construct it first with new AnytoneUart(context, selector)");
        return instance;
    }
    ...
    public AnytoneUart(Context ctx, String selector) throws IOException {
        ...
        if (!usbManager.hasPermission(device)) throw new IOException("Missing USB permission for device");
        ...
        synchronized (AnytoneUart.class) { instance = this; }
        // start async reading
        try { port.read(this::onReceivedData); } catch (Exception ignored) {}
        // Attach to CommsThread singleton if it's already created (or create lazy if not yet needed)
        try { CommsThread.attachUart(this); } catch (IllegalStateException ignored) { /* CommsThread will attach later */ }
    }
    ...
    void restartPort() throws IOException {
        ...
        if (!usbManager.hasPermission(device)) throw new IOException("Missing USB permission on restart");
        connection = usbManager.openDevice(device);
    ...
    int blockingRead(byte[] buf, int timeoutMs) throws IOException { // visibility relaxed from private
        ...
        synchronized (rxLock) {
            while (rxCount == 0) {
                ...
                try { rxLock.wait(Math.min(100, remain)); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); return 0; }
            }
        ...
        synchronized (rxLock) {
            while (off < len) {
                if (rxCount == 0) {
                    long remain = deadline - System.currentTimeMillis();
                    if (remain <= 0) break;
                    try { rxLock.wait(Math.min(100, remain)); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); break; }
                    continue;
                }
    ...
    public boolean parseFrom(long addr, byte[] resp, int len) {
        ...
        if ((resp[0] & 0xFF) == 0x57) { // 'W'
            ...
            if (len < 0 || len > 255) throw new IllegalArgumentException("tamanho inválido: " + len);
            ...
            if (resp.length < minNoAckLen) throw new IllegalArgumentException("faltam dados/checksum");
