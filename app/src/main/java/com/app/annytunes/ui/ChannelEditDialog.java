...
public final class ChannelEditDialog {
    ...
    public static void show(ChannelTransferActivity act, Channel ch, int position) {
        ...
        EditText rx = new EditText(act);
        rx.setHint("RX MHz");
        rx.setInputType(InputType.TYPE_CLASS_NUMBER | InputType.TYPE_NUMBER_FLAG_DECIMAL);
        rx.setText(hzToMHz(ch.rxHz));
        EditText tx = new EditText(act);
        tx.setHint("TX MHz");
        tx.setInputType(InputType.TYPE_CLASS_NUMBER | InputType.TYPE_NUMBER_FLAG_DECIMAL);
        tx.setText(hzToMHz(ch.txHz));
        ...
        mode.setSelection(ch.digital ? 0 : 1);
        Spinner ccSpin = new Spinner(act);
        String[] ccVals = new String[16];
        for (int i = 0; i < 16; i++) ccVals[i] = String.valueOf(i);
        ...
        ccSpin.setAdapter(ccAdapter);
        ccSpin.setSelection(Math.max(0, Math.min(15, ch.colorCode)));
        ...
        tsSpin.setAdapter(tsAdapter);
        tsSpin.setSelection(ch.timeslot == 1 ? 0 : 1);
        ...
        int admitIdx = 0;
        if ("CC Free".equalsIgnoreCase(ch.admit)) admitIdx = 1;
        else if ("Channel Free".equalsIgnoreCase(ch.admit)) admitIdx = 2;
        admitSpin.setSelection(admitIdx);
        ...
        powSpin.setAdapter(powAdapter);
        powSpin.setSelection(Math.max(0, Math.min(3, ch.power)));
        layout.addView(name);
        layout.addView(rx);
        layout.addView(tx);
        layout.addView(mode);
        layout.addView(ccSpin);
        layout.addView(tsSpin);
        layout.addView(admitSpin);
        layout.addView(powSpin);
        b.setView(layout);
    ...
    private static String hzToMHz(long hz) {
        if (hz <= 0) return "";
        double mhz = hz / 1_000_000.0;
        return String.format(Locale.getDefault(), "%.6f", mhz);
    }

    private static long mhzToHz(CharSequence cs) {
        try {
